# -*- coding: utf-8 -*-
"""Poke BattleES.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1svP-yCXIQApx-rXkQsjTA4oZnpVWb0EY

#Importing Libraries
"""
import pandas as pd
from pyknow import *
import random

# Reading all cards (including energy and legendary)
all_poke = pd.read_csv('PokewEng.csv')
# Getting only non-Legendary cards
regular_poke = all_poke[all_poke['Legendary'] == False]
regular_poke.reset_index(inplace=True)

global computerhp
global usehp
global attack_strategy
global user_cards
global computer_cards
global usercardselection
global roundnum
global console_output

computerhp = 1000
userhp = 1000
totalhp = 1000
attack_strategy = 0
user_cards = []
computer_cards = []
usercardselection = 125
roundnum = 1
console_output = ""
current_state = ""
legen_rounds = 0

"""#Expert System"""

# For reference - poke_card dict (According to card number)
'''
    col0 = Poke Name
    col1 = Type
    col2 = Attack
    col3 = Defense
'''

# For reference - Comparison of types
''' Water beats Fire
    Electric beats Water
    Grass beats Rock
    Rock beats Electric
    Fire beats Grass'''
# The Fact Subclasses
class PokeES(Fact):
    """
    Holds game Facts (moving from rule to rule)
    """
    pass
class RegularPokeCards(Fact):
    """
    The starting deck - no legendary Poke
    """
    p_key = Field(int, mandatory=True)
    p_name = Field(str, mandatory=True)
    p_type = Field(str, mandatory=True)
    p_attack = Field(int, mandatory=True)
    p_defense = Field(int, mandatory=True)
class AllPokeCards(Fact):
    """
    Info about the cards in the full deck
    """
    p_key = Field(int, mandatory=True)
    p_name = Field(str, mandatory=True)
    p_type = Field(str, mandatory=True)
    p_attack = Field(int, mandatory=True)
    p_defense = Field(int, mandatory=True)
class ComputerAttackDifficulty(Fact):
    """
    Will store the user's selection for the computer's difficulty.
    We have two options:
    EASIER - when attacking, the computer will select a card at random
    HARDER - the computer will always use its card with the strongest attack

    NOTE: the computer's defense choices will be determined by the Expert System
    """
    diff = Field(int, mandatory=True)
class DealtCards(Fact):
    """
    Will hold the 10 cards that are dealt.
    We will check if more than two energy cards were dealt out.
    If there were, we will redeal
    """
    pass
class UserCards(Fact):
    """
    Holds the user's cards for this round
    """
    pass
class ComputerCards(Fact):
    """
    Holds the computer's cards for this round
    """
    pass
class HP(Fact):
    """
    User and computer's overall health
    This will be updated each round
    """
    user = Field(int, mandatory=True)
    computer = Field(int, mandatory=True)
class LegendaryThreshold(Fact):
    """
    This will hold the threshold that, when hit, will trigger the legendary deck.
    It will be 25% of whatever is set as the starting HP.
    """
    pass
class LegendaryRounds(Fact):
    """
    Keeps track of when the legendary rounds are reached.
    The first time, a message will be displayed and the user will be given an 
    advantage if they are behind.  After that, the normal game will continue, 
    only with the additional cards.
    """
    leg_rounds = Field(int, mandatory=True)
class RoundNumber(Fact):
    """
    The round number will determine who goes first on any given draw
    Will be updated each round
    """
    round_num = Field(int, mandatory=True)
class UserCard(Fact):
    """
    The user's choice for the round
    This will hold the index of the card 
    From here, we can access the knowledge base to extract the type, attack, defense when needed
    """
    pass
class UserCardType(Fact):
    """
    This will hold the user's card type.  This will be the basis upon which the computer
    chooses its card to play.
    """
    pass
class ComputerCard(Fact):
    """
    The computer's choice for the round
    """
    pass
class ComputerCardTypes(Fact):
    """
    This will store the types of the computer's five cards during each round
    """
    pass
class ComputerCardType(Fact):
    """
    This will store the type of the computer's chosen card each round
    """
    pass
class UserEnergyMultiplier(Fact):
    """
    Will store the multiplier of the user's energy card, if they have played one
    Values will be 1.1, 1.2, or 1.3.
    If no energy card is played, it will be stored as 1. 
    """
    pass
class ComputerEnergyMultiplier(Fact):
    """
    Will store the multiplier of the computer's energy card, if they have played one
    Values will be 1.1, 1.2, or 1.3.
    If no energy card is played, it will be stored as 1. 
    """
    pass
class WhoGetsMultiplier(Fact):
    """
    This will store who is awarded the multiplier after the type comparison
    It will change round to round based on the type comparisons
    """
    pass
class Multiplier(Fact):
    """
    This will store what the multiplier is for the given round
    It will change based on the type comparisons 
    """
    pass
# The Knowledge Engine
class PlayPokeES(KnowledgeEngine):
    # Global variables to convey info back to webapp
    console_output = ""
    computerhp = computerhp
    userhp = userhp
    usercardselection = usercardselection
    user_cards = user_cards
    computer_cards = computer_cards
    round_num = roundnum
    current_state = current_state
    totalhp = totalhp
    legen_rounds = legen_rounds

    # Helper function to set values of global variables
    def set_values(self, console, chp, uhp, ucs, uc, cc, rn, state, thp, lrn):
        self.console_output = console
        self.computerhp = chp
        self.userhp = uhp
        self.usercardselection = ucs
        self.user_cards = uc
        self.computer_cards = cc
        self.round_num = rn
        self.current_state = state
        self.totalhp = thp
        self.legen_rounds = lrn

    # DefFacts is called every time the reset method is
    # This includes generators of the facts needed by the game
    # We store here all of the available Poke cards and their attributes
    @DefFacts()
    def game_settings(self):
        # Dictionary of avaiable regular Poke 
        self.poke_cards = dict()
        for i in range(len(regular_poke)):
            yield RegularPokeCards(p_key=int(regular_poke['#'][i]), p_name=regular_poke['Name'][i],
                                      p_type=regular_poke['Type 1'][i], p_attack=int(regular_poke['Attack'][i]),
                                      p_defense=int(regular_poke['Defense'][i]))

        # Dictionary of avaiable Poke (with Legendary added) 
        self.all_poke_cards = dict()
        for i in range(len(all_poke)):
            yield AllPokeCards(p_key=int(all_poke['#'][i]), p_name=all_poke['Name'][i],
                                  p_type=all_poke['Type 1'][i], p_attack=int(all_poke['Attack'][i]),
                                  p_defense=int(all_poke['Defense'][i]))

    # Store the dictionary of the regular poke cards
    @Rule(NOT(PokeES()), RegularPokeCards(p_key=MATCH.p_key, p_name=MATCH.name, p_type=MATCH.p_type,
                                                p_attack=MATCH.attack, p_defense=MATCH.defense))
    def def_poke_cards(self, p_key, name, p_type, attack, defense):
        self.poke_cards[p_key] = [name, p_type, attack, defense]

    # Store the dictionary of the full deck of poke cards
    @Rule(NOT(PokeES()), AllPokeCards(p_key=MATCH.p_key, p_name=MATCH.name, p_type=MATCH.p_type,
                                            p_attack=MATCH.attack, p_defense=MATCH.defense))
    def def_all_poke_cards(self, p_key, name, p_type, attack, defense):
        self.all_poke_cards[p_key] = [name, p_type, attack, defense]

    # Define what happens when the game is started
    @Rule()
    def new_game(self):
        self.console_output += 'Let\'s play PokeBattlES!'
        self.console_output += '<br> It\'s your turn to ' + ('ATTACK' if self.round_num % 2 != 0 else "DEFEND")
        total_hp = self.totalhp
        comp_hp = self.computerhp  # int(input('Enter Starting HP (min 200): '))
        human_hp = self.userhp
        # Set the starting values for user and computer's HP
        self.declare(HP(user=human_hp, computer=comp_hp))
        # Set the value that will trigger the legendary deck
        self.declare(LegendaryThreshold(int(total_hp * 0.25)))
        # Set the computer's attack difficulty level
        c_diff = attack_strategy  # int(input('Choose Computer\'s Attack Strategy (0 for random, 1 for optimal): '))
        # Store this fact
        self.declare(ComputerAttackDifficulty(diff=c_diff))
        # Reset to start at round one
        self.declare(RoundNumber(round_num=self.round_num))
        # Set the legendary round counter to 0
        self.declare(LegendaryRounds(leg_rounds=0))

        # Move to next rule
        self.declare(PokeES('show_scores'))

    # This is where we will begin each round as the game progresses
    # Showing scores and round number
    @Rule(AS.f1 << PokeES('show_scores'),
          AS.f2 << HP(user=MATCH.uhp,
                      computer=MATCH.chp),
          AS.f3 << RoundNumber(round_num=MATCH.rnum),
          AS.f4 << LegendaryThreshold(MATCH.legthresh))
    def print_current_scores(self, f1, f2, f3, uhp, chp, rnum, legthresh):
        self.retract(f1)
        # NOTE: we do not retract HP and Round Number because these are being maintained 
        # throughout the game
        if self.current_state != 'Draw':
            self.console_output += '<br>Round: ' + str(rnum)
            self.console_output += '<br>Current HP: '
            self.console_output += '<br>You: ' + str(uhp)
            self.console_output += '<br>Computer: ' + str(chp)
        # We want to check if the threshold to change the deck has been hit
        if uhp <= legthresh or chp <= legthresh:
            self.declare(PokeES('threshold_hit'))
        else:
            self.declare(PokeES('deal'))

    # If the legendary threshold is hit we introduce the legendary Poke into the deck
    @Rule(AS.f1 << PokeES('threshold_hit'),
          AS.f2 << LegendaryRounds(leg_rounds=MATCH.lrounds),
          AS.f3 << HP(user=MATCH.uhp,
                      computer=MATCH.chp))
    def thresh_hit(self, f1, f2, f3, lrounds, uhp, chp):
        self.retract(f1)
        # The first time it is hit, a message will be displayed
        if lrounds == 0:
            self.console_output += '<br>THE END GAME HAS BEEN REACHED'
            self.console_output += '<br>Legendary Poke have been added to the deck.'
            self.console_output += '<br>Legendary Poke receive a 1.5 multiplier against all regular Poke.'
            # Update the legendary rounds fact so this bonus isn't given again
            self.modify(f2, leg_rounds=lrounds + 1)
            self.legen_rounds = lrounds + 1
            # If the user is trailing at this point, they will be guaranteed to receive
            # a legendary Poke in the next round
            if uhp < chp:
                self.console_output += '<br>You are behind! Here is a bonus to help you catch up.'
                self.declare(PokeES('user_behind_legendary'))
            else:
                # Otherwise, both players will continue and the full deck will be used
                self.console_output += '<br>You are ahead! Keep going!'
                self.console_output += '<br>Dealing..............'
                self.declare(PokeES('deal_full_deck'))
        else:
            # Following the first end game round, it will continue to use the full deck
            self.console_output += '<br>Dealing..............'
            self.declare(PokeES('deal_full_deck'))

    # If the user is behind the first time the threshold is hit, they will be guaranteed
    # to receive a legendary Poke in the next round.
    @Rule(AS.f1 << PokeES('user_behind_legendary'))
    def user_behind_legendary(self, f1):
        self.retract(f1)
        self.console_output += '<br>Dealing..............'
        # Get the list of all Poke cards
        card_nums = list(self.all_poke_cards.keys())
        # Get the list of Legendary Poke cards
        leg_nums = []
        for card in card_nums:
            if self.all_poke_cards[card][1] == 'Legendary':
                leg_nums.append(card)
        # Give one of these randomly to the user
        u_bonus = random.choice(leg_nums)
        # Take this card out of the deck
        card_nums.remove(u_bonus)

        # Now, cards will be dealt randomly from the entire deck until each has five 
        # Select 9 random cards
        # We want to make sure none of these are energy cards since we are 
        # removing that bonus from this stage of the game
        energy_indexes = [200, 201, 202, 203, 204, 205, 206, 207, 208, 209, 210]
        while True:
            dealt_cards = random.sample(card_nums, 9)
            dealt_energies = 0
            for card in dealt_cards:
                if card in energy_indexes:
                    dealt_energies += 1
            if dealt_energies == 0:
                break
        # Now we have 9 cards that are not energy cards
        # We will alternate dealing - starting with the computer
        self.user_cards = []
        self.computer_cards = []

        # Add the user's legendary card to their hand
        self.user_cards.append(u_bonus)
        # Deal with the rest
        i = 0
        # Give one to the computer to even the hands
        self.computer_cards.append(dealt_cards[-1])
        while i < (len(dealt_cards) - 2):
            self.user_cards.append(dealt_cards[i])
            self.computer_cards.append(dealt_cards[i + 1])
            i += 2
        if self.current_state == 'Draw':
            return
        # Store the user and computer's cards for this round
        self.declare(UserCards(self.user_cards))
        self.declare(ComputerCards(self.computer_cards))
        # Move to show the user their cards
        self.declare(PokeES('show_user_cards'))

    # Deal regular cards to user and computer
    @Rule(AS.f1 << PokeES('deal'))
    def deal_cards(self, f1):
        self.retract(f1)
        # Get the list of all available cards
        card_nums = list(self.poke_cards.keys())
        if len(self.user_cards) == 5 and len(self.computer_cards) == 5:
            dealt_cards = []
            i = 0
            while i < (len(self.user_cards)):
                dealt_cards.append(self.user_cards[i])
                dealt_cards.append(self.computer_cards[i])
                i += 1
        else:
            # Select 10 random cards
            dealt_cards = random.sample(card_nums, 10)

        # Store the cards
        self.declare(DealtCards(dealt_cards))
        self.declare(PokeES('check_deal'))

    # We want to ensure no more than two energy cards were dealt out 
    @Rule(AS.f1 << PokeES('check_deal'),
          AS.f2 << DealtCards(MATCH.dealt_cards))
    def check_deal(self, f1, f2, dealt_cards):
        self.retract(f1)
        energy_indexes = [200, 201, 202, 203, 204, 205, 206, 207, 208, 209, 210]
        dealt_energies = 0
        for card in dealt_cards:
            if card in energy_indexes:
                dealt_energies += 1

        # Either redeal, or move on with these cards
        if dealt_energies <= 2:
            self.declare(PokeES('split_cards'))
        else:
            # Throw away the current cards because we are going to redeal
            self.retract(f2)
            self.declare(PokeES('deal'))

    # Now that we have 10 cards, we want to split them between the user and the computer
    @Rule(AS.f1 << PokeES('split_cards'),
          AS.f2 << DealtCards(MATCH.dealt_cards))
    def split_cards(self, f1, f2, dealt_cards):
        self.retract(f1)
        self.retract(f2)
        # Here we will deal these cards - alternating back and forth between players
        self.user_cards.clear()
        self.computer_cards.clear()
        i = 0
        while i < (len(dealt_cards)):
            self.user_cards.append(dealt_cards[i])
            self.computer_cards.append(dealt_cards[i + 1])
            i += 2

        if self.current_state == 'Draw':
            return
        # Store the user and computer's cards for this round
        self.declare(UserCards(self.user_cards))
        self.declare(ComputerCards(self.computer_cards))
        self.declare(PokeES('show_user_cards'))

    # Deal the full deck of cards to user and computer
    @Rule(AS.f1 << PokeES('deal_full_deck'))
    def deal_full_deck(self, f1):
        self.retract(f1)
        # Get the list of all available cards
        card_nums = list(self.all_poke_cards.keys())
        # Select 10 random cards
        dealt_cards = random.sample(card_nums, 10)
        # Store the cards
        self.declare(DealtCards(dealt_cards))
        self.declare(PokeES('check_deal_leg'))

    # We want to ensure no more than two energy cards were dealt out 
    @Rule(AS.f1 << PokeES('check_deal_leg'),
          AS.f2 << DealtCards(MATCH.dealt_cards))
    def check_deal_leg(self, f1, f2, dealt_cards):
        self.retract(f1)
        energy_indexes = [200, 201, 202, 203, 204, 205, 206, 207, 208, 209, 210]
        dealt_energies = 0
        for card in dealt_cards:
            if card in energy_indexes:
                dealt_energies += 1

        # Either redeal, or move on with these cards
        if dealt_energies == 0:
            self.declare(PokeES('split_cards'))
        else:
            # Throw away the current cards because we are going to redeal
            self.retract(f2)
            self.declare(PokeES('deal_full_deck'))

    # Show the user their cards
    @Rule(AS.f1 << PokeES('show_user_cards'), UserCards(MATCH.u_cards))
    def show_user_cards(self, f1, u_cards):
        self.retract(f1)
        # self.console_output += '<br>Here are your cards'
        # self.console_output += '............'
        # Print the user their available cards, along with the attributes of each
        # for i in u_cards:
        #     self.console_output += '<br>' + str(i) + ' ' + self.all_poke_cards[i][0]
        #     print(i, self.all_poke_cards[i][0])
        #     self.console_output += '<br>Type: {}, Attack: {}, Defense: {}'.format(self.all_poke_cards[i][1],
        #                                                                           self.all_poke_cards[i][2],
        #                                                                           self.all_poke_cards[i][3])
        #     self.console_output += '...................'
        #     print('Type: {}, Attack: {}, Defense: {}'.format(self.all_poke_cards[i][1],
        #                                                      self.all_poke_cards[i][2],
        #                                                      self.all_poke_cards[i][3]))
        #     print('.....................')
        self.declare(PokeES('whose_turn'))

    # Determine whose turn it is
    @Rule(AS.f1 << PokeES('whose_turn'), RoundNumber(round_num=MATCH.rnum))
    def whose_turn(self, f1, rnum):
        self.retract(f1)
        # We will have the player go first on odd numbered rounds, and the computer
        # on even numbered rounds
        if rnum % 2 != 0:
            self.declare(PokeES('user_plays_first'))
        else:
            self.declare(PokeES('computer_plays_first'))

    ####
    #
    # CASE 1:  the player goes first (odd numbered rounds)
    #
    ####
    # When the player goes first, they are free to play any card they wish
    @Rule(AS.f1 << PokeES('user_plays_first'),
          AS.f2 << UserCards(MATCH.u_cards))
    def user_plays_first(self, f1, f2, u_cards):
        self.retract(f1)
        self.console_output += '<br>It\'s your turn.'
        self.console_output += '<br>You are on the ATTACK!'
        self.console_output += '<br>Select a card to play.'
        # We get the user's selection
        # We handle if they enter a value corresponding to a card they do not have
        while True:
            choice = int(self.usercardselection)  # int(input('Enter card number: '))
            if choice in list(u_cards):
                break
            else:
                if choice in self.all_poke_cards:
                    break
                else:
                    self.usercardselection = list(u_cards)[0]
                #self.console_output += '<br>You don\'t have that card!'
        self.console_output += '<br>You play {}!'.format(self.all_poke_cards[choice][0])

        # If the user plays an energy card
        if self.all_poke_cards[choice][1] == 'Energy':
            self.console_output += '<br>YOU GET AN ENERGY BOOST!'
            # Store the boost multiplier
            if self.all_poke_cards[choice][0] == 'Energy 10':
                self.declare(UserEnergyMultiplier(1.1))
            elif self.all_poke_cards[choice][0] == 'Energy 20':
                self.declare(UserEnergyMultiplier(1.2))
            else:
                self.declare(UserEnergyMultiplier(1.3))
            # The user now must make another choice
            self.declare(PokeES('user_first_energy'))

        else:
            # The user did not play an energy card
            # We are finished with the user's set of cards for this round
            self.retract(f2)
            # Store the user's selection and move to computer
            self.declare(UserCard(choice))
            # Store the type of the user's card choice
            self.declare(UserCardType(self.all_poke_cards[choice][1]))
            self.declare(UserEnergyMultiplier(1))
            self.declare(PokeES('computer_plays_second'))

    @Rule(AS.f1 << PokeES('user_first_energy'),
          AS.f2 << UserCards(MATCH.u_cards))
    def user_first_energy(self, f1, f2, u_cards):
        self.retract(f1)
        self.retract(f2)
        u_cards = list(u_cards)
        energy_indexes = [200, 201, 202, 203, 204, 205, 206, 207, 208, 209, 210]
        poke = []
        # Only one energy card can be played per turn
        for card in u_cards:
            if card not in energy_indexes:
                poke.append(card)
        # We get the user's selection
        # We handle if they enter a value corresponding to a card they do not have
        while True:
            choice = self.usercardselection  # int(input('Select your Poke: '))
            if choice in poke:
                break
            else:
                if choice in self.all_poke_cards:
                    break
                else:
                    self.usercardselection = poke[0]
                # self.console_output += '<br>You don\'t have that card!'
        self.console_output += '<br>You play {}!'.format(self.all_poke_cards[choice][0])
        # Store the user's selection and move to computer
        self.declare(UserCard(choice))
        # Store the type of the user's card choice
        self.declare(UserCardType(self.all_poke_cards[choice][1]))
        self.declare(PokeES('computer_plays_second'))

    ####
    # COMPUTER PLAYS SECOND
    ####

    # The computer will make its decision based on the type of the user's card
    @Rule(AS.f1 << PokeES('computer_plays_second'),
          AS.f2 << ComputerCards(MATCH.c_cards))
    def computer_plays_second(self, f1, f2, c_cards):
        self.retract(f1)
        # First, we want to store the computer's card types
        c_types = []
        for card in c_cards:
            c_types.append(self.all_poke_cards[card][1])

        # We will store this as a fact
        self.declare(ComputerCardTypes(c_types))

        if 'Energy' in c_types:
            self.declare(PokeES('computer_second_energy'))
        else:
            self.declare(ComputerEnergyMultiplier(1))
            self.declare(PokeES('computer_defense'))

    @Rule(AS.f1 << PokeES('computer_second_energy'),
          AS.f2 << ComputerCards(MATCH.c_cards))
    def computer_second_energy(self, f1, f2, c_cards):
        self.retract(f1)
        self.console_output += '<br>THE COMPUTER GETS AN ENERGY BOOST!'
        e10 = [160, 161, 162, 163, 164]
        e20 = [165, 166]
        e30 = [167, 168, 169]

        # Checking to see the best energy card the computer has
        # This is what they will play
        c_cards = list(c_cards)
        for card in e30:
            if card in c_cards:
                self.declare(ComputerEnergyMultiplier(1.3))
                break
        for card in e20:
            if card in c_cards:
                self.declare(ComputerEnergyMultiplier(1.2))
                break
        for card in e10:
            if card in c_cards:
                self.declare(ComputerEnergyMultiplier(1.1))
                break
        self.declare(PokeES('computer_defense'))

    @Rule(AS.f1 << PokeES('computer_defense'),
          AS.f2 << UserCardType(MATCH.u_type),
          AS.f3 << ComputerCards(MATCH.c_cards))
    def computer_defense(self, f1, f2, f3, u_type, c_cards):
        # Then the computer will base its action on the its best matchup 
        # NOTE:  It will always take into account the multiplier (based on types)
        # and then also see if it has a better option. 
        self.retract(f1)
        if u_type == 'Fire':
            self.declare(PokeES('comp_vs_fire'))
        elif u_type == 'Water':
            self.declare(PokeES('comp_vs_water'))
        elif u_type == 'Grass':
            self.declare(PokeES('comp_vs_grass'))
        elif u_type == 'Electric':
            self.declare(PokeES('comp_vs_electric'))
        elif u_type == 'Rock':
            self.declare(PokeES('comp_vs_rock'))
        elif u_type == 'Legendary':
            self.declare(PokeES('comp_vs_legendary'))

    @Rule(AS.f1 << PokeES('comp_vs_fire'),
          AS.f2 << ComputerCardTypes(MATCH.c_types),
          AS.f3 << ComputerCards(MATCH.c_cards))
    def comp_vs_fire(self, f1, f2, f3, c_types, c_cards):
        # Here, water will give a bonus
        # We will see if the computer has a water Poke, calculate the bonus 
        # And then see if it can do better with its other cards.
        self.retract(f1)
        self.retract(f2)
        self.retract(f3)
        # Get the indexes of all the computer's water Poke
        water = [i for i, x in enumerate(c_types) if x == 'Water']
        # For each of these, get corresponding Poke
        water_poke = []
        for i in water:
            water_poke.append(c_cards[i])
        # For each of these, get the defense statistics
        water_defenses = []
        for i in water_poke:
            water_defenses.append(self.all_poke_cards[i][3])

        # If the computer has water Poke, get the max defense
        if water_defenses:
            # Get the highest of these water defense values
            max_water_def = max(water_defenses)
            # Get the index of this within this list
            max_water_def_index = water_defenses.index(max_water_def)
            # Get the actual Poke card # of this best water defense
            best_water = water_poke[max_water_def_index]
        else:
            # The computer has no water Poke
            max_water_def = 0
        # We now want to check if the computer has a better option
        # Since water beats fire, this will be awarded the 1.2 multiplier
        max_water_def = int(max_water_def * 1.2)

        # Get the computer's non-water Poke
        comp_others = []
        for i in c_cards:
            if i not in water_poke:
                comp_others.append(i)
        # Get the defense statistics for these
        # Remember, the a legendary will get a 1.5 times boost vs all regular cards
        other_defs = []
        for i in comp_others:
            if self.all_poke_cards[i][1] == 'Legendary':
                other_defs.append(int(self.all_poke_cards[i][3] * 1.5))
            else:
                other_defs.append(self.all_poke_cards[i][3])
        # Get the highest value
        if other_defs:
            other_defs_max = max(other_defs)
        else:
            other_defs_max = 0
        # Now, make the comparison
        if max_water_def >= other_defs_max:
            # The computer will play its best water Poke
            # Store the card number
            self.declare(ComputerCard(best_water))
            # Move to the compare types stage
            self.declare(PokeES('comp_play'))
        else:
            # The computer has a better play than its best water
            # Get index of its best defense
            best_other_ind = other_defs.index(other_defs_max)
            # Get the Poke card number
            best_other = comp_others[best_other_ind]
            # Store the card number
            self.declare(ComputerCard(best_other))
            # Move to the compare types stage
            self.declare(PokeES('comp_play'))

    @Rule(AS.f1 << PokeES('comp_vs_water'),
          AS.f2 << ComputerCardTypes(MATCH.c_types),
          AS.f3 << ComputerCards(MATCH.c_cards))
    def comp_vs_water(self, f1, f2, f3, c_types, c_cards):
        # Here, electric will give a bonus
        # We will see if the computer has an electric Poke, calculate the bonus 
        # And then see if it can do better with its other cards.
        self.retract(f1)
        self.retract(f2)
        self.retract(f3)
        # Get the indexes of all the computer's electric Poke
        electric = [i for i, x in enumerate(c_types) if x == 'Electric']
        # For each of these, get corresponding Poke
        electric_poke = []
        for i in electric:
            electric_poke.append(c_cards[i])
        # For each of these, get the defense statistics
        electric_defenses = []
        for i in electric_poke:
            electric_defenses.append(self.all_poke_cards[i][3])

        # If the computer has electric Poke, get the max defense
        if electric_defenses:
            # Get the highest of these electric defense values
            max_electric_def = max(electric_defenses)
            # Get the index of this within this list
            max_electric_def_index = electric_defenses.index(max_electric_def)
            # Get the actual Poke card # of this best electric defense
            best_electric = electric_poke[max_electric_def_index]
        else:
            # The computer has no electric Poke
            max_electric_def = 0
        # We now want to check if the computer has a better option
        # Since electric beats water, this will be awarded the 1.2 multiplier
        max_electric_def = int(max_electric_def * 1.2)

        # Get the computer's non-electric Poke
        comp_others = []
        for i in c_cards:
            if i not in electric_poke:
                comp_others.append(i)
        # Get the defense statistics for these
        # Remember, the a legendary will get a 1.5 times boost vs all regular cards
        other_defs = []
        for i in comp_others:
            if self.all_poke_cards[i][1] == 'Legendary':
                other_defs.append(int(self.all_poke_cards[i][3] * 1.5))
            else:
                other_defs.append(self.all_poke_cards[i][3])
        # Get the highest value
        if other_defs:
            other_defs_max = max(other_defs)
        else:
            other_defs_max = 0
        # Now, make the comparison
        if max_electric_def >= other_defs_max:
            # The computer will play its best electric Poke
            # Store the card number
            self.declare(ComputerCard(best_electric))
            # Move to the compare types stage
            self.declare(PokeES('comp_play'))
        else:
            # The computer has a better play than its best electric
            # Get index of its best defense
            best_other_ind = other_defs.index(other_defs_max)
            # Get the Poke card number
            best_other = comp_others[best_other_ind]
            # Store the card number
            self.declare(ComputerCard(best_other))
            # Move to the compare types stage
            self.declare(PokeES('comp_play'))

    @Rule(AS.f1 << PokeES('comp_vs_grass'),
          AS.f2 << ComputerCardTypes(MATCH.c_types),
          AS.f3 << ComputerCards(MATCH.c_cards))
    def comp_vs_grass(self, f1, f2, f3, c_types, c_cards):
        # Here, fire will give a bonus
        # We will see if the computer has any fire Poke, calculate the bonus 
        # And then see if it can do better with its other cards.
        self.retract(f1)
        self.retract(f2)
        self.retract(f3)
        # Get the indexes of all the computer's fire Poke
        fire = [i for i, x in enumerate(c_types) if x == 'Fire']
        # For each of these, get corresponding Poke
        fire_poke = []
        for i in fire:
            fire_poke.append(c_cards[i])
        # For each of these, get the defense statistics
        fire_defenses = []
        for i in fire_poke:
            fire_defenses.append(self.all_poke_cards[i][3])

        # If the computer has fire Poke, get the max defense
        if fire_defenses:
            # Get the highest of these fire defense values
            max_fire_def = max(fire_defenses)
            # Get the index of this within this list
            max_fire_def_index = fire_defenses.index(max_fire_def)
            # Get the actual Poke card # of this best fire defense
            best_fire = fire_poke[max_fire_def_index]
        else:
            # The computer has no fire Poke
            max_fire_def = 0
        # We now want to check if the computer has a better option
        # Since fire beats grass, this will be awarded the 1.2 multiplier
        max_fire_def = int(max_fire_def * 1.2)

        # Get the computer's non-fire Poke
        comp_others = []
        for i in c_cards:
            if i not in fire_poke:
                comp_others.append(i)
        # Get the defense statistics for these
        # Remember, the a legendary will get a 1.5 times boost vs all regular cards
        other_defs = []
        for i in comp_others:
            if self.all_poke_cards[i][1] == 'Legendary':
                other_defs.append(int(self.all_poke_cards[i][3] * 1.5))
            else:
                other_defs.append(self.all_poke_cards[i][3])
        # Get the highest value
        if other_defs:
            other_defs_max = max(other_defs)
        else:
            other_defs_max = 0
        # Now, make the comparison
        if max_fire_def >= other_defs_max:
            # The computer will play its best fire Poke
            # Store the card number
            self.declare(ComputerCard(best_fire))
            # Move to the compare types stage
            self.declare(PokeES('comp_play'))
        else:
            # The computer has a better play than its best fire
            # Get index of its best defense
            best_other_ind = other_defs.index(other_defs_max)
            # Get the Poke card number
            best_other = comp_others[best_other_ind]
            # Store the card number
            self.declare(ComputerCard(best_other))
            # Move to the compare types stage
            self.declare(PokeES('comp_play'))

    @Rule(AS.f1 << PokeES('comp_vs_electric'),
          AS.f2 << ComputerCardTypes(MATCH.c_types),
          AS.f3 << ComputerCards(MATCH.c_cards))
    def comp_vs_electric(self, f1, f2, f3, c_types, c_cards):
        # Here, rock will give a bonus
        # We will see if the computer has any rock Poke, calculate the bonus 
        # And then see if it can do better with its other cards.
        self.retract(f1)
        self.retract(f2)
        self.retract(f3)
        # Get the indexes of all the computer's rock Poke
        rock = [i for i, x in enumerate(c_types) if x == 'Rock']
        # For each of these, get corresponding Poke
        rock_poke = []
        for i in rock:
            rock_poke.append(c_cards[i])
        # For each of these, get the defense statistics
        rock_defenses = []
        for i in rock_poke:
            rock_defenses.append(self.all_poke_cards[i][3])

        # If the computer has rock Poke, get the max defense
        if rock_defenses:
            # Get the highest of these rock defense values
            max_rock_def = max(rock_defenses)
            # Get the index of this within this list
            max_rock_def_index = rock_defenses.index(max_rock_def)
            # Get the actual Poke card # of this best rock defense
            best_rock = rock_poke[max_rock_def_index]
        else:
            # The computer has no rock Poke
            max_rock_def = 0
        # We now want to check if the computer has a better option
        # Since rock beats electric, this will be awarded the 1.2 multiplier
        max_rock_def = int(max_rock_def * 1.2)

        # Get the computer's non-rock Poke
        comp_others = []
        for i in c_cards:
            if i not in rock_poke:
                comp_others.append(i)
        # Get the defense statistics for these
        # Remember, the a legendary will get a 1.5 times boost vs all regular cards
        other_defs = []
        for i in comp_others:
            if self.all_poke_cards[i][1] == 'Legendary':
                other_defs.append(int(self.all_poke_cards[i][3] * 1.5))
            else:
                other_defs.append(self.all_poke_cards[i][3])
        # Get the highest value
        if other_defs:
            other_defs_max = max(other_defs)
        else:
            other_defs_max = 0
        # Now, make the comparison
        if max_rock_def >= other_defs_max:
            # The computer will play its best rock Poke
            # Store the card number
            self.declare(ComputerCard(best_rock))
            # Move to the compare types stage
            self.declare(PokeES('comp_play'))
        else:
            # The computer has a better play than its best rock
            # Get index of its best defense
            best_other_ind = other_defs.index(other_defs_max)
            # Get the Poke card number
            best_other = comp_others[best_other_ind]
            # Store the card number
            self.declare(ComputerCard(best_other))
            # Move to the compare types stage
            self.declare(PokeES('comp_play'))

    @Rule(AS.f1 << PokeES('comp_vs_rock'),
          AS.f2 << ComputerCardTypes(MATCH.c_types),
          AS.f3 << ComputerCards(MATCH.c_cards))
    def comp_vs_rock(self, f1, f2, f3, c_types, c_cards):
        # Here, grass will give a bonus
        # We will see if the computer has any grass Poke, calculate the bonus 
        # And then see if it can do better with its other cards.
        self.retract(f1)
        self.retract(f2)
        self.retract(f3)
        # Get the indexes of all the computer's grass Poke
        grass = [i for i, x in enumerate(c_types) if x == 'Grass']
        # For each of these, get corresponding Poke
        grass_poke = []
        for i in grass:
            grass_poke.append(c_cards[i])
        # For each of these, get the defense statistics
        grass_defenses = []
        for i in grass_poke:
            grass_defenses.append(self.all_poke_cards[i][3])

        # If the computer has grass Poke, get the max defense
        if grass_defenses:
            # Get the highest of these grass defense values
            max_grass_def = max(grass_defenses)
            # Get the index of this within this list
            max_grass_def_index = grass_defenses.index(max_grass_def)
            # Get the actual Poke card # of this best grass defense
            best_grass = grass_poke[max_grass_def_index]
        else:
            # The computer has no grass Poke
            max_grass_def = 0
        # We now want to check if the computer has a better option
        # Since grass beats rock, this will be awarded the 1.2 multiplier
        max_grass_def = int(max_grass_def * 1.2)

        # Get the computer's non-grass Poke
        comp_others = []
        for i in c_cards:
            if i not in grass_poke:
                comp_others.append(i)
        # Get the defense statistics for these
        # Remember, the a legendary will get a 1.5 times boost vs all regular cards
        other_defs = []
        for i in comp_others:
            if self.all_poke_cards[i][1] == 'Legendary':
                other_defs.append(int(self.all_poke_cards[i][3] * 1.5))
            else:
                other_defs.append(self.all_poke_cards[i][3])
        # Get the highest value
        if other_defs:
            other_defs_max = max(other_defs)
        else:
            other_defs_max = 0
        # Now, make the comparison
        if max_grass_def >= other_defs_max:
            # The computer will play its best grass Poke
            # Store the card number
            self.declare(ComputerCard(best_grass))
            # Move to the compare types stage
            self.declare(PokeES('comp_play'))
        else:
            # The computer has a better play than its best grass
            # Get index of its best defense
            best_other_ind = other_defs.index(other_defs_max)
            # Get the Poke card number
            best_other = comp_others[best_other_ind]
            # Store the card number
            self.declare(ComputerCard(best_other))
            # Move to the compare types stage
            self.declare(PokeES('comp_play'))

    @Rule(AS.f1 << PokeES('comp_vs_legendary'),
          AS.f2 << ComputerCardTypes(MATCH.c_types),
          AS.f3 << ComputerCards(MATCH.c_cards))
    def comp_vs_legendary(self, f1, f2, f3, c_types, c_cards):
        # Here, there is no bonus to be had
        # A legendary Poke will counter the user's legendary, so we will check 
        # 1.5 times that defense against the other defense statistics
        self.retract(f1)
        self.retract(f2)
        self.retract(f3)
        # Get all the defense statistics
        comp_defenses = []
        for card in c_cards:
            if self.all_poke_cards[card][1] == 'Legendary':
                comp_defenses.append(int(self.all_poke_cards[card][3] * 1.5))
            else:
                comp_defenses.append(self.all_poke_cards[card][3])
        # Get the maximum value
        max_def = max(comp_defenses)
        # Get the index of this
        best_index = comp_defenses.index(max_def)
        # Get the Poke number
        best_def = c_cards[best_index]
        # Store the card number
        self.declare(ComputerCard(best_def))
        # Move to the compare types stage
        self.declare(PokeES('comp_play'))

    @Rule(AS.f1 << PokeES('comp_play'),
          AS.f2 << ComputerCard(MATCH.c_card))
    def comp_play_second(self, f1, f2, c_card):
        self.retract(f1)
        # Display the computer's choice
        self.console_output += '<br>Computer plays {}!'.format(self.all_poke_cards[c_card][0])
        self.console_output += '<br>Type: {}, Attack: {}, Defense: {}'.format(self.all_poke_cards[c_card][1],
                                                                              self.all_poke_cards[c_card][2],
                                                                              self.all_poke_cards[c_card][3])
        self.declare(PokeES('compare_types'))

    ####
    #
    # CASE 2:  the computer goes first (even numbered rounds)
    #
    ####
    @Rule(AS.f1 << PokeES('computer_plays_first'),
          AS.f2 << ComputerCards(MATCH.c_cards))
    def computer_plays_first(self, f1, f2, c_cards):
        # The first thing we will do is see if the computer has an engery card.
        self.retract(f1)
        # First, we want to store the computer's card types
        c_types = []
        for card in c_cards:
            c_types.append(self.all_poke_cards[card][1])

        if 'Energy' in c_types:
            self.declare(PokeES('computer_first_energy'))
        else:
            self.declare(ComputerEnergyMultiplier(1))
            self.declare(PokeES('computer_attack_style'))

    @Rule(AS.f1 << PokeES('computer_first_energy'),
          AS.f2 << ComputerCards(MATCH.c_cards))
    def computer_first_energy(self, f1, f2, c_cards):
        self.retract(f1)
        self.console_output += '<br>THE COMPUTER GETS AN ENERGY BOOST!'
        e10 = [160, 161, 162, 163, 164]
        e20 = [165, 166]
        e30 = [167, 168, 169]

        # Checking to see the best energy card the computer has
        # This is what they will play
        c_cards = list(c_cards)
        for card in e30:
            if card in c_cards:
                self.declare(ComputerEnergyMultiplier(1.3))
                break
        for card in e20:
            if card in c_cards:
                self.declare(ComputerEnergyMultiplier(1.2))
                break
        for card in e10:
            if card in c_cards:
                self.declare(ComputerEnergyMultiplier(1.1))
                break
        self.declare(PokeES('computer_attack_style'))

    @Rule(AS.f1 << PokeES('computer_attack_style'),
          AS.f2 << ComputerAttackDifficulty(diff=MATCH.c_diff))
    def computer_attack_style(self, f1, f2, c_diff):
        self.retract(f1)
        # If the user selected 0, the computer will attack by selecting random cards
        if c_diff == 0:
            self.declare(PokeES('comp_attack_random'))
        else:
            # Otherwise, the computer will look at its cards and choose the best one
            self.declare(PokeES('comp_attack_optimal'))

    @Rule(AS.f1 << PokeES('comp_attack_random'),
          AS.f2 << ComputerCards(MATCH.c_cards))
    def comp_attack_random(self, f1, f2, c_cards):
        self.retract(f1)
        self.retract(f2)
        # Get the list of card numbers
        c_cards = list(c_cards)
        # Make a random choice
        c_choice = int(random.choice(c_cards))
        # Get the Poke name
        c_card = self.all_poke_cards[c_choice][0]
        # Display the information for the user
        self.console_output += '<br>Computer plays {}!'.format(c_card)
        self.console_output += '<br>Type: {}, Attack: {}, Defense: {}'.format(self.poke_cards[c_choice][1],
                                                                              self.poke_cards[c_choice][2],
                                                                              self.poke_cards[c_choice][3])
        # Store the computer's choice
        self.declare(ComputerCard(c_choice))
        # Move to the user
        self.declare(PokeES('user_plays_second'))

    ###
    #
    # THE COMPUTER WILL PLAY A LEGENDARY CARD, IF IT HAS ONE.
    # IF NOT, IT WILL PLAY THE CARD WITH THE BEST ATTACK.
    #
    ###
    @Rule(AS.f1 << PokeES('comp_attack_optimal'),
          AS.f2 << ComputerCards(MATCH.c_cards))
    def comp_attack_optimal(self, f1, f2, c_cards):
        self.retract(f1)
        # Get the types of the computer's cards
        comp_types = []
        for card in c_cards:
            comp_types.append(self.all_poke_cards[card][1])
        # If the computer has a legendary card, it will play it
        if 'Legendary' in comp_types:
            self.retract(f2)
            leg_ind = comp_types.index('Legendary')
            # Get the card number
            c_card = c_cards[leg_ind]
            # Display the info for the user
            self.console_output += '<br>Computer played ' + self.all_poke_cards[c_card][0]
            self.console_output += '<br>Type: {}, Attack: {}, Defense: {}'.format(self.all_poke_cards[c_card][1],
                                                                                  self.all_poke_cards[c_card][2],
                                                                                  self.all_poke_cards[c_card][3])
            # Store the computer's choice
            self.declare(ComputerCard(c_card))
            # Move to the user
            self.declare(PokeES('user_plays_second'))
        else:
            self.declare(PokeES('comp_plays_no_legendary'))

    @Rule(AS.f1 << PokeES('comp_plays_no_legendary'),
          AS.f2 << ComputerCards(MATCH.c_cards))
    def comp_plays_first_no_legendary(self, f1, f2, c_cards):
        self.retract(f1)
        self.retract(f2)
        # The computer will play the card with the highest attack value
        comp_attacks = []
        for card in c_cards:
            comp_attacks.append(self.all_poke_cards[card][2])
        # Get the highest value
        max_attack = max(comp_attacks)
        # Get the index of this
        max_ind = comp_attacks.index(max_attack)
        # Get the corresponding Poke card number
        c_card = c_cards[max_ind]
        # Display information for the user
        self.console_output += '<br>Computer plays ' + self.all_poke_cards[c_card][0] + '!'
        self.console_output += '<br>Type: ' + self.all_poke_cards[c_card][1] + ', Attack: ' + \
                               self.all_poke_cards[c_card][2] + ', Defense: ' + self.all_poke_cards[c_card][3]
        # Store the computer's choice
        self.declare(ComputerCard(c_card))
        # Move to the user
        self.declare(PokeES('user_plays_second'))

    # The user sees the computer's play and it is now their turn
    @Rule(AS.f1 << PokeES('user_plays_second'),
          AS.f2 << UserCards(MATCH.u_cards))
    def user_plays_second(self, f1, f2, u_cards):
        self.retract(f1)
        self.console_output += '<br>It\'s your turn to DEFEND!'
        # We get the user's selection
        # We handle if they enter a value corresponding to a card they do not have 
        while True:
            choice = int(self.usercardselection)  # int(input('Enter card number: '))
            if choice in list(u_cards):
                break
            else:
                if choice in self.all_poke_cards:
                    break
                else:
                    self.usercardselection = list(u_cards)[0]
                # self.console_output += '<br>You don\'t have that card!'
        self.console_output += '<br>You played ' + self.all_poke_cards[choice][0]

        # If the user plays an energy card
        if self.all_poke_cards[choice][1] == 'Energy':
            self.console_output += '<br>YOU GET AN ENERGY BOOST!'
            # Store the boost multiplier
            if self.all_poke_cards[choice][0] == 'Energy 10':
                self.declare(UserEnergyMultiplier(1.1))
            elif self.all_poke_cards[choice][0] == 'Energy 20':
                self.declare(UserEnergyMultiplier(1.2))
            else:
                self.declare(UserEnergyMultiplier(1.3))
            # The user now must make another choice
            self.declare(PokeES('user_second_energy'))

        else:
            # The user did not play an energy card
            # We are finished with the user's set of cards for this round
            self.retract(f2)
            # Store the user's selection and move to computer
            self.declare(UserCard(choice))
            # Store the type of the user's card choice
            self.declare(UserCardType(self.all_poke_cards[choice][1]))
            self.declare(UserEnergyMultiplier(1))
            self.declare(PokeES('compare_types'))

    @Rule(AS.f1 << PokeES('user_second_energy'),
          AS.f2 << UserCards(MATCH.u_cards))
    def user_second_energy(self, f1, f2, u_cards):
        self.retract(f1)
        self.retract(f2)
        u_cards = list(u_cards)
        energy_indexes = [200, 201, 202, 203, 204, 205, 206, 207, 208, 209, 210]
        poke = []
        # Only one energy card can be played per turn
        for card in u_cards:
            if card not in energy_indexes:
                poke.append(card)
        # We get the user's selection
        # We handle if they enter a value corresponding to a card they do not have
        while True:
            choice = int(self.usercardselection)  # int(input('Select your Poke: '))
            if choice in poke:
                break
            else:
                if choice in self.all_poke_cards:
                    break
                else:
                    self.usercardselection = poke[0]
                # self.console_output += '<br>You don\'t have that card!'
        self.console_output += '<br>You played ' + self.all_poke_cards[choice][0]
        # Store the user's selection and move to computer
        self.declare(UserCard(choice))
        # Store the type of the user's card choice
        self.declare(UserCardType(self.all_poke_cards[choice][1]))
        self.declare(PokeES('compare_types'))

    # Comparing Poke types
    # This will determine the multiplier bonus
    @Rule(AS.f1 << PokeES('compare_types'),
          AS.f2 << UserCardType(MATCH.u_type),
          AS.f3 << ComputerCard(MATCH.c_card))
    def compare_types(self, f1, f2, f3, u_type, c_card):
        self.retract(f1)
        # Get the computer's card type
        c_type = self.all_poke_cards[c_card][1]
        self.console_output += '<br>Your Poke type is ' + u_type.upper()
        self.console_output += '<br>The computer\'s Poke type is ' + c_type.upper()
        self.declare(ComputerCardType(c_type))

        if u_type == 'Legendary' and c_type == 'Legendary':
            self.declare(PokeES('both_legendary'))
        elif u_type == 'Legendary' and c_type != 'Legendary':
            self.declare(PokeES('user_legendary'))
        elif u_type != 'Legendary' and c_type == 'Legendary':
            self.declare(PokeES('comp_legendary'))
        else:
            self.declare(PokeES('no_legendary'))

    @Rule(AS.f1 << PokeES('both_legendary'),
          AS.f2 << UserCardType(MATCH.u_type),
          AS.f3 << ComputerCardType(MATCH.c_type))
    def both_legendary(self, f1, f2, f3, u_type, c_type):
        self.retract(f1)
        self.retract(f2)
        self.retract(f3)
        bonus = None
        # Store which player received the bonus
        self.declare(WhoGetsMultiplier(bonus))
        self.console_output += '<br>Two legendary Poke! There is no bonus.'
        # Store the multiplier value
        multiplier = 1
        self.declare(Multiplier(multiplier))
        # Move next to the battle
        self.declare(PokeES('battle'))

    @Rule(AS.f1 << PokeES('user_legendary'),
          AS.f2 << UserCardType(MATCH.u_type),
          AS.f3 << ComputerCardType(MATCH.c_type))
    def user_legendary(self, f1, f2, f3, u_type, c_type):
        self.retract(f1)
        self.retract(f2)
        self.retract(f3)
        bonus = 'user_legendary'
        # Store which player received the bonus
        self.declare(WhoGetsMultiplier(bonus))
        self.console_output += '<br>You get a large boost!'
        # Store the multiplier value
        multiplier = 1.5
        self.declare(Multiplier(multiplier))
        # Move next to the battle
        self.declare(PokeES('battle'))

    @Rule(AS.f1 << PokeES('comp_legendary'),
          AS.f2 << UserCardType(MATCH.u_type),
          AS.f3 << ComputerCardType(MATCH.c_type))
    def comp_legendary(self, f1, f2, f3, u_type, c_type):
        self.retract(f1)
        self.retract(f2)
        self.retract(f3)
        bonus = 'comp_legendary'
        # Store which player received the bonus
        self.declare(WhoGetsMultiplier(bonus))
        self.console_output += 'The computer gets a large boost!'
        # Store the multiplier value
        multiplier = 1.5
        self.declare(Multiplier(multiplier))
        # Move next to the battle
        self.declare(PokeES('battle'))

    @Rule(AS.f1 << PokeES('no_legendary'),
          AS.f2 << UserCardType(MATCH.u_type),
          AS.f3 << ComputerCardType(MATCH.c_type))
    def no_legendary(self, f1, f2, f3, u_type, c_type):
        self.retract(f1)
        self.retract(f2)
        self.retract(f3)
        # Make the comparisons
        if u_type == 'Fire':
            if c_type == 'Grass':
                bonus = 'user'
            elif c_type == 'Water':
                bonus = 'computer'
            else:
                bonus = None

        elif u_type == 'Water':
            if c_type == 'Fire':
                bonus = 'user'
            elif c_type == 'Electric':
                bonus = 'computer'
            else:
                bonus = None

        elif u_type == 'Grass':
            if c_type == 'Rock':
                bonus = 'user'
            elif c_type == 'Fire':
                bonus = 'computer'
            else:
                bonus = None

        elif u_type == 'Rock':
            if c_type == 'Electric':
                bonus = 'user'
            elif c_type == 'Grass':
                bonus = 'computer'
            else:
                bonus = None

        elif u_type == 'Electric':
            if c_type == 'Water':
                bonus = 'user'
            elif c_type == 'Rock':
                bonus = 'computer'
            else:
                bonus = None
        else:
            bonus = None

        # Set the bonus value
        multiplier = 1.2
        # Store which player received the bonus
        self.declare(WhoGetsMultiplier(bonus))
        # Store the multiplier value
        self.declare(Multiplier(multiplier))
        # Print the bonus status
        if bonus == 'user':
            self.console_output += '<br>You get a boost!'
        elif bonus == 'computer':
            self.console_output += '<br>The computer gets a boost!'
        else:
            self.console_output += '<br>There is no bonus.'

        # Move next to the battle
        self.declare(PokeES('battle'))

    # We are now ready to determine the damage done in the round
    # We need the card selections, bonus information, and the round number
    # Round number is being used to keep track of who plays first in the round
    @Rule(AS.f1 << PokeES('battle'),
          AS.f2 << UserCard(MATCH.u_card),
          AS.f3 << ComputerCard(MATCH.c_card),
          AS.f4 << WhoGetsMultiplier(MATCH.bonus),
          AS.f5 << Multiplier(MATCH.multiplier),
          AS.f6 << UserEnergyMultiplier(MATCH.ueng),
          AS.f7 << ComputerEnergyMultiplier(MATCH.ceng),
          AS.f8 << HP(user=MATCH.uhp, computer=MATCH.chp),
          RoundNumber(round_num=MATCH.rnum))
    def battle(self, f1, f2, f3, f4, f5, f6, f7, f8, u_card, c_card, bonus,
               multiplier, ueng, ceng, uhp, chp, rnum):
        self.retract(f1)
        self.retract(f2)
        self.retract(f3)
        self.retract(f4)
        self.retract(f5)
        self.retract(f6)
        self.retract(f7)
        # Do not retract HP and round numbers - these will be updated at the 
        # end of the round
        ###
        #
        # FOR NOW: whoever goes first, always attacks
        # Other player always defends
        # User goes first on odd numbered rounds
        ###
        self.console_output += '<br>............'
        self.console_output += '<br>BATTLE!'
        self.console_output += '<br>............'
        # On odd rounds, the user goes first
        if rnum % 2 != 0:
            # Get the attack value of their chosen card
            u_attack = self.all_poke_cards[u_card][2]
            # Get the computer's defense
            c_defense = self.all_poke_cards[c_card][3]
            if bonus == 'user':
                u_attack = int(u_attack * multiplier)
            elif bonus == 'user_legendary':
                u_attack = int(u_attack * multiplier)
            elif bonus == 'computer':
                c_defense = int(c_defense * multiplier)
            elif bonus == 'comp_legendary':
                c_defense = int(c_defense * multiplier)
            # Apply the energy multiplier
            u_attack = int(u_attack * ueng)
            c_defense = int(c_defense * ceng)
            # Display the attack and defense stats with bonus awarded
            self.console_output += '<br>You attack with ' + str(u_attack)
            self.console_output += '<br>The computer defends with ' + str(c_defense)
            # Check how user's attack does against computer's defense
            difference = u_attack - c_defense
            # If attack is higher than defense, user is successful
            if difference > 0:
                self.console_output += '<br>Success! You did ' + str(difference) + ' damage!'
                # Remove the damage from the computer's CURRENT overall HP
                chp = chp - difference
            elif difference == 0:
                self.console_output += '<br>A perfect matchup. No damage!'
            else:
                # If defense is stronger, the user uses the damage
                # NOTE:  here difference is a negative number
                self.console_output += '<br>Your attack was defended! You lost ' + str(abs(difference)) + ' HP!'
                uhp = uhp + difference

        # On even numbered rounds, the computer goes first
        else:
            # As above, award bonus
            c_attack = self.all_poke_cards[c_card][2]
            u_defense = self.all_poke_cards[u_card][3]
            if bonus == 'computer':
                c_attack = int(c_attack * multiplier)
            elif bonus == 'comp_legendary':
                c_attack = int(c_attack * multiplier)
            elif bonus == 'user':
                u_defense = int(u_defense * multiplier)
            elif bonus == 'user_legendary':
                u_defense = int(u_defense * multiplier)

            # Apply the energy multiplier
            c_attack = int(c_attack * ceng)
            u_defense = int(u_defense * ueng)
            self.console_output += '<br>The computer attacks with ' + str(c_attack)
            self.console_output += '<br>You defend with ' + str(u_defense)

            # See if computer's attack is successful
            difference = c_attack - u_defense
            if difference > 0:
                self.console_output += '<br>The computer did ' + str(difference) + ' damage to you!'
                uhp = uhp - difference
            elif difference == 0:
                self.console_output += '<br>A perfect matchup. No damage!'
            else:
                self.console_output += '<br>You defended the attack! The computer lost ' + str(difference) + ' HP!'
                chp = chp + difference

        self.userhp = uhp
        self.computerhp = chp
        # Modify the fact to update the overall HP standings for both players
        self.modify(f8, user=uhp, computer=chp)

        # We are continuing the game until one of the players reaches zero HP
        # Check if that has happened in this round
        self.round_num += 1
        if uhp <= 0 or chp <= 0:
            # If it has, move to end game
            self.declare(PokeES('end_game'))
        else:
            # If it hasn't, start a new round
            self.declare(PokeES('next_round'))

    # As the game continues, we start a new round 
    # All new cards will be dealt 
    @Rule(AS.f1 << PokeES('next_round'),
          AS.f2 << RoundNumber(round_num=MATCH.rnum))
    def next_round(self, f1, f2, rnum):
        self.retract(f1)
        self.console_output += '<br>Next Round beginning'
        # Update the round number fact 
        self.modify(f2, round_num=rnum + 1)
        return
        # roundnum = rnum
        # # Move to the show_scores function that begins the round
        # # Round and scores and displayed and the game is repeated
        # self.declare(PokeES('show_scores'))

    # We reach here when one of the players has reached zero HP            
    @Rule(AS.f1 << PokeES('end_game'),
          AS.f2 << HP(user=MATCH.uhp, computer=MATCH.chp))
    def end_the_game(self, f1, f2, uhp, chp):
        self.retract(f1)
        self.retract(f2)

        roundnum = -1
        # If it is the computer that has reached zero, the player wins
        if chp <= 0:
            self.console_output += '<br>You win! You are a Poke Master :)!'
        else:
            self.console_output += '<br>You lost! Better luck next time!'
# Helper method to Instantiate Pyknow
def runIt(hp_val, ai_val, uc, ac, rn, selectedid, state, thp, lrn):
    # Initialize the knowledge engine
    computerhp = ai_val
    userhp = hp_val
    user_cards = uc
    computer_cards = ac
    roundnum = rn
    usercardselection = selectedid
    current_state = state
    game = PlayPokeES()
    game.set_values('', ai_val, hp_val, selectedid, uc, ac, rn, state, thp, lrn)
    # Reset and run
    game.reset()
    game.run()
    return game.console_output, game.computerhp, game.userhp, game.user_cards, game.computer_cards, game.round_num, game.legen_rounds
